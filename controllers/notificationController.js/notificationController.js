...
exports.updateStatus = async (req, res) => {
  ...
  console.log(newData);
  try {
    const updatePromises = newData.map(
      async ({ id, user_id, message, sendAll }) => {
        if (sendAll) {
          return Notification.updateMany(
            { book: id, user: user_id, sendAll: true },
            {
              $set: {
          ...
          return Notification.updateMany(
            { book: id, user: user_id, sendAll: false },
            {
              $set: {
                notificationStatus: 1,
                messageNotification: message,
...
exports.sendNotificationsToALl = async (req, res) => {
